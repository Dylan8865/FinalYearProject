  ...
  ({ 
    ...
    useImperativeHandle(ref, () => ({
      reset: () => {
        ...
        targetPositionRef.current.set(defaultPosition[0], defaultPosition[1], defaultPosition[2]);
        targetLookAtRef.current.set(defaultTarget[0], defaultTarget[1], defaultTarget[2]);
        setIsAnimating(true);
      },
    }), [defaultPosition, defaultTarget]);
    ...
    useFrame(() => {
      ...
      const positionDistance = camera.position.distanceTo(defaultPosVec.current);
      const targetDistance = controlsRef.current 
        ? controlsRef.current.target.distanceTo(defaultTargetVec.current) 
        : 0;
      
      const isCurrentlyAtDefault = positionDistance < 0.5 && targetDistance < 0.5;
      ...
      if (controlsRef.current) {
        controlsRef.current.target.lerp(targetLookAtRef.current, transitionSpeed);
        controlsRef.current.update();
      }

      // Check if we've reached the animation target (within a small threshold)
      const animPosDistance = camera.position.distanceTo(targetPositionRef.current);
