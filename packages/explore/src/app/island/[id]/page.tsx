...
export default async function IslandPage({ params }: Props) {
    ...
    if (islandError) {
        return <div className="p-8 text-center text-red-400">Error loading island: {islandError.message}</div>
    }
        ...
        <div className="h-screen overflow-y-auto bg-gray-950 text-white p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                {islandItems.map((item) => (
                    <div key={item.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-blue-500/50 transition-colors">
                        <div className="p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-lg">{item.title || item.item?.name || 'Untitled Item'}</h3>
                                    <p className="text-xs text-gray-500 uppercase tracking-wider">{item.item?.type}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                {!itemDataMap[item.id]?.length && (
                                    <p className="text-gray-600 italic text-sm">No knowledge content yet.</p>
                                )}
                            </div>
                        </div>
                    </div>
...
function renderBlockContent(block: ItemDataType) {
    switch (block.type) {
        case 'paragraph':
            return <p>{block.content?.text || (typeof block.content === 'string' ? block.content : '')}</p>
        case 'heading_1':
            return <h4 className="text-xl font-bold text-white mt-2">{block.content?.text}</h4>
        case 'heading_2':
            return <h5 className="text-lg font-bold text-white mt-2">{block.content?.text}</h5>
        case 'bulleted_list':
            return <li className="ml-4 list-disc">{block.content?.text}</li>
        case 'image':
            return block.content?.url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={block.content.url} alt="Content" className="rounded-lg w-full mt-2" />
            ) : null
        case 'code':
            return (
