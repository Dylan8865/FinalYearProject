  ...
  return (
    <div className={`min-h-screen p-8 ${bgColor}`}>
      <div className="mx-auto max-w-6xl">
        ...
        {/* Most Helpful Topics */}
        <div className={`mb-8 rounded-lg border ${borderColor} ${cardBg} p-6`}>
          <div className="mb-4 flex items-center justify-between">
            <h2 className={`text-xl font-semibold ${textColor}`}>Most Helpful Topics</h2>
            <span className={`text-sm ${mutedTextColor}`}>Your top-rated conversations</span>
          </div>
          
          {!analytics.mostHelpfulTopics || analytics.mostHelpfulTopics.length === 0 ? (
            <div className={`text-center py-8 ${mutedTextColor}`}>
              No helpful topics yet. Give positive feedback to see them here!
            </div>
          ) : (
            <div className="space-y-3">
              {analytics.mostHelpfulTopics.map((topic, index) => (
                <div
                  key={topic.chatId}
                  className={`flex items-center justify-between rounded-lg border ${borderColor} p-4 transition-colors hover:border-teal-500`}
                >
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span className={`text-sm ${mutedTextColor}`}>
                            {topic.positiveCount} helpful {topic.positiveCount === 1 ? 'response' : 'responses'}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  {/* Quick Action Button */}
                  <a
                    href={`/search?chatId=${topic.chatId}`}
                    className="ml-4 flex items-center gap-2 rounded-lg bg-teal-500 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-teal-600"
                  >
              ...
              {analytics.recentFeedback.map((feedback) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className={`font-medium ${textColor}`}>{feedback.chatTitle}</div>
