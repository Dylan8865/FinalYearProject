...
import { FiAlertCircle, FiArrowLeft, FiArrowRight, FiBox, FiHeart, FiRefreshCw, FiSearch, FiShare2, FiStar, FiTrash2, FiX } from 'react-icons/fi';
...
export default function ModelLibraryPage() {
  ...
  const [pickModel, setPickModel] = useState<ThreeDModelSummary | null>(null);
  ...
  useEffect(() => {
    ...
    authService.getMyEducatorPicks().then((picks) => {
      setMyEducatorPicks(Object.fromEntries(picks.filter((pick) => pick.target_type === 'model').map((pick) => [pick.target_id, pick.recommendation_id])));
    }).catch(() => undefined);
  }, [user?.role]);
  ...
  const submitEducatorPick = async () => {
    if (!pickModel || !pickNote.trim() || isSavingPick) return;
    setIsSavingPick(true);
    try {
      const saved = await authService.createEducatorPick('model', pickModel.resource_id, pickNote.trim());
      setMyEducatorPicks((current) => ({ ...current, [pickModel.resource_id]: saved.recommendation_id }));
      setPickModel(null);
      setPickNote('');
    } finally {
  ...
  const toggleEducatorPick = async (model: ThreeDModelSummary) => {
    const recommendationId = myEducatorPicks[model.resource_id];
    if (!recommendationId) { setPickModel(model); setPickNote(''); return; }
    await authService.deleteEducatorPick(recommendationId);
    setMyEducatorPicks((current) => { const next = { ...current }; delete next[model.resource_id]; return next; });
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] text-slate-950 lg:grid lg:grid-cols-[260px_1fr] lg:grid-rows-[auto_1fr]">
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-8 md:px-8 lg:py-10">
        ...
        {!isLoading && !error && visibleModels.length > 0 && <section className="mt-8 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{visibleModels.map((model) => <article key={model.resource_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg"><div className="relative aspect-video overflow-hidden bg-[radial-gradient(circle_at_50%_30%,#dbeafe_0%,#eff6ff_40%,#f8fafc_75%)]">{model.is_deleted && <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/70 backdrop-blur-[2px]"><span className="rounded-xl bg-rose-100 px-3 py-1.5 text-sm font-bold text-rose-700 shadow-sm">❌ Removed by admin</span></div>}{model.is_locked && !model.is_deleted && <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/70 backdrop-blur-[2px]"><span className="rounded-xl bg-amber-100 px-3 py-1.5 text-sm font-bold text-amber-700 shadow-sm">🔒 Locked by admin</span></div>}<ModelThumbnail modelUrl={model.preview_model_url} title={model.title} /></div><div className="p-5"><div className="flex items-start justify-between gap-3">{model.subject_name && <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-primary">{model.subject_name}</span>}<div className="flex gap-2">{!model.is_deleted && !model.is_locked && <button onClick={() => { setShareModel(model); setShareStatus(''); }} className="flex h-9 w-9 items-center justify-center rounded-xl bg-slate-100 text-slate-500 hover:bg-blue-50 hover:text-primary" aria-label={`Share ${model.title}`} title="Share inside Qubo"><FiShare2 /></button>}{!model.is_deleted && !model.is_locked && user?.role !== 'educator' && <button onClick={() => toggleFavourite(model.resource_id)} className={`flex h-9 w-9 items-center justify-center rounded-xl ${favouriteIds.has(model.resource_id) ? 'bg-rose-50 text-rose-500' : 'bg-slate-100 text-slate-500 hover:text-rose-500'}`} aria-label={`Save ${model.title}`}><FiHeart className={favouriteIds.has(model.resource_id) ? 'fill-current' : ''} /></button>}{!model.is_deleted && !model.is_locked && user?.role === 'educator' && <button onClick={() => void toggleEducatorPick(model)} aria-label={`Recommend ${model.title} to students`} title={myEducatorPicks[model.resource_id] ? 'Remove educator recommendation' : 'Recommend to students'} className={`flex h-9 w-9 items-center justify-center rounded-xl ${myEducatorPicks[model.resource_id] ? 'bg-amber-50 text-amber-600' : 'bg-slate-100 text-slate-500 hover:bg-amber-50 hover:text-amber-600'}`}><FiStar className={myEducatorPicks[model.resource_id] ? 'fill-current' : ''} /></button>}{user?.role === 'educator' && model.created_by === user.id && !model.is_deleted && <button onClick={() => void deleteModel(model)} className="flex h-9 w-9 items-center justify-center rounded-xl bg-red-50 text-red-500 hover:bg-red-100" aria-label={`Delete ${model.title}`} title="Delete your model"><FiTrash2 /></button>}</div></div><h2 className="mt-3 text-lg font-extrabold leading-6 text-slate-950">{model.title}</h2><p className="mt-2 text-sm text-slate-500">{model.topic_name || 'Interactive 3D learning model'}</p><button onClick={() => navigate(`/models/${model.resource_id}`)} disabled={model.is_deleted} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-bold text-white shadow-lg shadow-blue-600/20 hover:bg-blue-700 disabled:opacity-45 disabled:cursor-not-allowed">{model.is_locked ? 'Edit annotations' : 'Explore model'} <FiArrowRight /></button></div></article>)}</section>}
      </main>
      ...
      {pickModel && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Recommend ${pickModel.title}`}><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-amber-600">Educator's pick</p><h2 className="mt-1 text-lg font-extrabold text-slate-950">Recommend this model</h2></div><button onClick={() => setPickModel(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100" aria-label="Close recommendation dialog"><FiX /></button></div><label className="mt-5 block text-sm font-bold text-slate-700">Why should students explore this?<textarea value={pickNote} onChange={(event) => setPickNote(event.target.value)} maxLength={300} rows={4} className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3 py-2.5 font-medium outline-none focus:border-amber-400 focus:ring-2 focus:ring-amber-100" placeholder="e.g. This model clearly shows the bonding structure of water molecules." autoFocus /></label><button onClick={submitEducatorPick} disabled={!pickNote.trim() || isSavingPick} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 px-4 py-3 font-bold text-white shadow-lg shadow-amber-500/20 disabled:opacity-60"><FiStar /> {isSavingPick ? 'Saving…' : 'Recommend to students'}</button></div></div>}
    </div>
  );
}
