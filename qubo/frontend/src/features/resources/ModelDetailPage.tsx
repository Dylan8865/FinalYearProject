    ...
    <div className="min-h-screen bg-[#f7f9fb] text-slate-950 lg:grid lg:grid-cols-[260px_1fr] lg:grid-rows-[auto_1fr]">
      ...
      <main className="mx-auto w-full max-w-7xl px-5 py-8 md:px-8 lg:py-10">
        ...
        {model && !isLoading && !error && <div className="mt-6 grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section>
            <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.2em] text-primary">Interactive model</p><h1 className="mt-2 text-3xl font-extrabold tracking-tight text-slate-950 md:text-4xl">{model.title}</h1></div><div className="flex gap-2">{isEducator && !model.is_locked && !model.is_deleted && <button onClick={() => educatorPick ? void removeEducatorPick() : setIsPickDialogOpen(true)} className={`inline-flex h-12 items-center justify-center gap-2 rounded-2xl border px-4 text-sm font-bold transition ${educatorPick ? 'border-amber-200 bg-amber-50 text-amber-700' : 'border-slate-200 bg-white text-slate-600 hover:bg-amber-50 hover:text-amber-700'}`} title={educatorPick ? 'Remove educator recommendation' : 'Recommend to students'}><FiStar className={educatorPick ? 'fill-current' : ''} /><span className="hidden sm:inline">{educatorPick ? 'Recommended' : 'Recommend'}</span></button>}{!model.is_locked && !model.is_deleted && <button onClick={() => { setShareStatus(''); setIsShareDialogOpen(true); }} className="flex h-12 w-12 flex-none items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 transition hover:border-blue-100 hover:bg-blue-50 hover:text-primary" aria-label={`Share ${model.title}`} title="Share inside Qubo"><FiShare2 className="h-5 w-5" /></button>}{!isEducator && !model.is_locked && !model.is_deleted && <button onClick={toggleFavourite} disabled={isSavingFavourite} className={`flex h-12 w-12 flex-none items-center justify-center rounded-2xl border transition disabled:opacity-60 ${isFavourite ? 'border-rose-100 bg-rose-50 text-rose-500' : 'border-slate-200 bg-white text-slate-500 hover:border-rose-100 hover:bg-rose-50 hover:text-rose-500'}`} aria-label={isFavourite ? `Remove ${model.title} from saved items` : `Save ${model.title}`} title={isFavourite ? 'Remove from Saved' : 'Save for revision'}><FiHeart className={`h-5 w-5 ${isFavourite ? 'fill-current' : ''}`} /></button>}</div></div>
