...
export default function TutorialVideoPage() {
  ...
  const submitShare = async () => {
    if (!shareVideo || !recipientUsername.trim()) return;
    setIsSharing(true);
    setShareStatus('');
    try {
      await authService.shareTutorialVideo(shareVideo.video_id, recipientUsername.trim(), shareMessage.trim() || undefined);
  ...
  return (
    <div className="min-h-screen bg-[#f7f9fb] text-slate-950 lg:grid lg:grid-cols-[260px_1fr]">
            ...
            <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {visibleVideos.map((video) => (
                <article key={video.video_id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                  <button onClick={() => openVideo(video)} className="group relative block aspect-video w-full overflow-hidden bg-slate-900 text-left">
      ...
      {shareVideo && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/70 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label={`Share ${shareVideo.title}`}><div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-extrabold uppercase tracking-[0.16em] text-primary">Share inside Qubo</p><h2 className="mt-1 text-lg font-extrabold text-slate-950">{shareVideo.title}</h2></div><button onClick={() => setShareVideo(null)} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100" aria-label="Close share dialog"><FiX /></button></div><label className="mt-5 block text-sm font-bold text-slate-700">Recipient username<input value={recipientUsername} onChange={(event) => setRecipientUsername(event.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2.5 font-medium outline-none focus:border-primary focus:ring-2 focus:ring-blue-100" placeholder="e.g. alina_student" autoFocus /></label><label className="mt-4 block text-sm font-bold text-slate-700">Message <span className="font-medium text-slate-400">(optional)</span><textarea value={shareMessage} onChange={(event) => setShareMessage(event.target.value)} maxLength={300} rows={3} className="mt-2 w-full resize-none rounded-xl border border-slate-200 px-3 py-2.5 font-medium outline-none focus:border-primary focus:ring-2 focus:ring-blue-100" placeholder="This may help with revision." /></label>{shareStatus && <p className={`mt-4 rounded-xl p-3 text-sm font-semibold ${shareStatus === 'Shared successfully.' ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{shareStatus}</p>}<button onClick={submitShare} disabled={!recipientUsername.trim() || isSharing} className="mt-5 flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-4 py-3 font-bold text-white shadow-lg shadow-blue-600/20 disabled:opacity-60"><FiShare2 /> {isSharing ? 'Sharing…' : 'Share video'}</button></div></div>}
